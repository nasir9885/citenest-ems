...
const [hasAttendance, setHasAttendance] =
  useState(false);
  ...
  async function loadAttendance(
    ...
    try {
      const response = await fetch(
        `/api/ems/attendance?date=${encodeURIComponent(
          selectedDate
        )}`,
        {
          cache: "no-store",
...
setHasAttendance(result.hasAttendance);
      const rows =
...
async function createAttendance() {
  ...
  try {
    ...
    const rows = result.employees.map(
      (employee: AttendanceEmployee) => ({
        ...employee,
        status: "PRESENT",
        check_in: "",
      ...
      <header className="ems-header">
        <div className="ems-container ems-nav">
          ...
          <div className="ems-language-switch">
            <Link href="/ems/en/attendance">
              English
            </Link>

            <Link href="/ems/ar/attendance">
              العربية
            </Link>
          </div>
