...
export default function BankExportPage() {
  ...
  const [records, setRecords] = useState<SalaryExportRecord[]>([]);
  ...
  async function loadRecords(selectedMonth = month, selectedYear = year) {
    ...
    try {
      const response = await fetch(
        `/api/ems/bank-export?month=${selectedMonth}&year=${selectedYear}`,
        {
          cache: "no-store",
        },
      );
      ...
      if (!response.ok) {
        setError(result.message || "Unable to load salary records.");
  ...
  const totalNetSalary = useMemo(
    () =>
      records.reduce(
        (total, record) => total + Number(record.net_salary || 0),
        0,
      ),
    [records],
  );
  ...
  function downloadCsv() {
    ...
    link.href = `/api/ems/bank-export?month=${month}&year=${year}&format=csv`;
    link.download = "";
      ...
      <header className="ems-header">
        <div className="ems-container ems-nav">
          ...
          <div className="ems-language-switch">
            <Link href="/ems/en/bank-export">English</Link>

            <Link href="/ems/ar/bank-export">العربية</Link>
          </div>
        </div>
      </header>
        ...
        <div className="ems-container">
          <div className="ems-page-toolbar">
            <div>
              ...
              <h1>{t.bankSalaryExport}</h1>

              <p className="ems-subtitle">{t.selectMonthYear}</p>
            </div>
          </div>
          ...
          <div className="salary-toolbar">
            ...
            <div className="salary-total">
              ...
              <strong>{totalNetSalary.toFixed(3)}</strong>
            </div>
          ...
          {!loading && !error && records.length === 0 && (
            <div className="ems-message">{t.noSalaryRecords}</div>
          )}
          ...
          {!loading && records.length > 0 && (
            <div className="ems-table-wrapper">
              <table className="ems-table bank-export-table">
                ...
                <tbody>
                  {records.map((record) => (
                    <tr key={record.employee_number}>
                      <td>
                        <strong>{record.employee_number}</strong>
                      </td>
                      ...
                      <td className="ems-number salary-net">
                        {Number(record.net_salary).toFixed(3)}
                      </td>
